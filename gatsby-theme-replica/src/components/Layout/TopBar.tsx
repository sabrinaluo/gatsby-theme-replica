...
const TopBar: FC = () => {
  ...
  return (
    <div
      className={`bg-gray-dark p-4 md:py-0 md:px-6`}
    >
      <div className={`w-full flex justify-between md:justify-auto items-center  md:h-60px`}>
      ...
      <NavList
        className={`${isExpanded ? '' : 'hidden'} w-full font-semibold`}
        itemClassName={`py-2 text-white hover:no-underline block border-t-fade-white15`}
      />
      {/*<Link to={'/'} className={`py-2 text-white hover:no-underline flex items-center border-t-fade-white15`}>*/}
